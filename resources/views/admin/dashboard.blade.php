...
@section('main')
    ...
    <script type="text/javascript" src="{{ asset('js/xlsx.full.min.js?ver=1.1') }}"></script>
        ...
        function app2() {
            ...
            data = data.map((n, i) => {
                ...
                @if ($settings->selectable_max > 0)
                    for (var x = 0; x < {{ $settings->selectable_max }}; x++) {
                        e['Admission Option ' + (x + 1)] = n.institutions[x] ? n.institutions[x].name : '';
                    }
                    var last = institutions.length + 2;
                    // current column
                    var col = String.fromCharCode(65 + Object.keys(e).length + 1);
                    var row = i + 2;
                    e["Allotment Id"] = {
                        t: 'n',
                        f: `IFERROR(INDEX(Institutions!$A$2:$A$${last},MATCH(Applications!${col}${row},Institutions!$B$2:$B$${last},0)),"")`
                    };
                    e["Allotment"] = n.allotted_institution ? n.allotted_institution.name : '';
                @else
                    e["Status"] = (n.status * 1) ? 'Selected' : 'Not Selected';
                @endif
                return e;
            });
            ...
            return {
                ...
                exportXLSX() {
                    ...
                    @if ($settings->selectable_max > 0)
                        const worksheet2 = XLSX.utils.json_to_sheet(this.institutions.map(i => {
                            return {
                                "Id": i.id,
                                "Institution": i.name
                            };
                        }));
                        // worksheet2 will be hidden
                        XLSX.utils.book_append_sheet(workbook, worksheet2, "Institutions");
                        XLSX.utils.book_set_sheet_visibility(workbook, "Institutions", 1);
                    @endif
                    ...
                    var last_col = String.fromCharCode(65 + worksheet["!cols"].length - 1);
                    var last_row = data.length + 1;
                    var sec_last_col = String.fromCharCode(65 + worksheet["!cols"].length - 2);
                    @if ($settings->selectable_max > 0)
                        worksheet["!cols"][worksheet["!cols"].length - 2].hidden = true;
                    @endif
                    worksheet['!dataValidation'] = [
                        @if ($settings->selectable_max > 0)
                            {
                                sqref: `${last_col}2:${last_col}${last_row}`,
                                values: `Institutions!$B$2:$B$${this.institutions.length + 1}`
                            }, {
                                sqref: `${sec_last_col}2:${sec_last_col}${last_row}`,
                                values: `Institutions!$A$2:$A$${this.institutions.length + 1}`
                            }
                        @else
                            {
                                sqref: `${last_col}2:${last_col}${last_row}`,
                    ...
                    XLSX.writeFile(workbook, `DHCMD-Applications-${this.currentFilter}-2023-24.xlsx`);
                },
